...
class MangaService {
  ...
  static Future<String?> _findMangaPillRoute(String title) async {
    try {
      ...
      final res = await http
          .get(Uri.parse('$_mpBase/search?q=$query'), headers: _mpHeaders)
  ...
  static Future<List<MangaChapter>> _getMangaPillChapters(
    String route,
    String mangaTitle,
  ) async {
    try {
      final res = await http
          .get(Uri.parse('$_mpBase$route'), headers: _mpHeaders)
          .timeout(const Duration(seconds: 15));
      ...
      for (final match in matches) {
        ...
        final rawTitle = match
            .group(2)!
            .replaceAll(RegExp(r'<[^>]*>'), '')
            .trim();

        String chapNum = '';
        final chapMatch = RegExp(
          r'chapter\s+([0-9.]+|[0-9]+-[0-9]+)',
          caseSensitive: false,
        ).firstMatch(rawTitle);
        if (chapMatch != null) {
  ...
  static Future<List<MangaPage>> _getMangaPillPages(String chapterRoute) async {
    try {
      final res = await http
          .get(Uri.parse('$_mpBase$chapterRoute'), headers: _mpHeaders)
  ...
  static Future<String?> _fetchDxByMalId(int malId) async {
    final url =
        '$_dxBase/manga?links[mal]=$malId&limit=5&order[relevance]=desc';
    try {
      final res = await http
          .get(Uri.parse(url), headers: _dxHeaders)
          .timeout(const Duration(seconds: 15));
      if (res.statusCode != 200) return null;
  ...
  static Future<String?> _fetchDxByTitle(String title) async {
    final url =
        '$_dxBase/manga?title=${Uri.encodeComponent(title)}&limit=5&order[relevance]=desc';
    try {
      final res = await http
          .get(Uri.parse(url), headers: _dxHeaders)
          .timeout(const Duration(seconds: 15));
  ...
  static Future<List<MangaChapter>> _getMangaDexChapters(
    String mangaDexId,
  ) async {
    try {
      final countRes = await http
          .get(
            Uri.parse(
              '$_dxBase/manga/$mangaDexId/feed?translatedLanguage[]=en&limit=1&offset=0&order[chapter]=asc',
            ),
            headers: _dxHeaders,
          )
          .timeout(const Duration(seconds: 15));
      ...
      final futures = List.generate(pageCount, (i) {
        return http
            .get(
              Uri.parse(
                '$_dxBase/manga/$mangaDexId/feed?translatedLanguage[]=en&limit=500&offset=${i * 500}&order[chapter]=asc',
              ),
              headers: _dxHeaders,
            )
          ...
          allChapters.add(
            MangaChapter(
              ...
              publishedAt: attrs['publishAt'] != null
                  ? DateTime.tryParse(attrs['publishAt'].toString())
                  : null,
  ...
  static Future<List<MangaPage>> _getMangaDexPages(String chapterId) async {
    try {
      ...
      final res = await http
          .get(Uri.parse('$_dxBase/at-home/server/$id'), headers: _dxHeaders)
          .timeout(const Duration(seconds: 20));
      if (res.statusCode != 200) return [];
  ...
  static bool isExternalChapter(String chapterId) =>
      chapterId.startsWith('mangadex-ext|');
