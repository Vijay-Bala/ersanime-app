...
class MusicPlaylistDetailScreen extends StatefulWidget {
  ...
  @override
  State<MusicPlaylistDetailScreen> createState() =>
      _MusicPlaylistDetailScreenState();
}
    ...
    return SliverAppBar(
      ...
      actions: [
        if (widget.overridePlaylist == null) ...[
          IconButton(
            icon: Icon(
              _reorderMode ? Icons.check_rounded : Icons.edit_rounded,
              color: _musicPrimary,
            ),
                      ...
                      Expanded(
                        child: OutlinedButton.icon(
                          ...
                          onPressed: playlist.songs.isEmpty
                              ? null
                              : () => _play(context, playlist.songs, 0),
                        ),
                      ),
                      SizedBox(width: 12.w),
                      Expanded(
  ...
  Widget _buildSongList(MusicPlaylist playlist, MusicPlaylistService service) {
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        return Dismissible(
          key: Key(song.id),
          direction: widget.overridePlaylist != null
          ...
          onDismissed: (_) =>
              service.removeSongFromPlaylist(playlist.id, song.id),
          child: SongListTile(
            song: song,
